...
def save_file_to_db(pk, filename, file_data):
    insert_query = sql.SQL("INSERT INTO files (pk, filename, file_data) VALUES ({}, {}, {})").format(
        sql.Literal(pk),
...
#     select_query = sql.SQL("SELECT filename, file_data FROM files WHERE pk = {}").format(
#         sql.Literal(pk),
#     )
...
#         return FilePayload(pk=pk, filename=filename, file_data=file_data)
#     else:
#         return None
